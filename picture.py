...
if __name__ == "__main__":
    input_folder = "D:\\python\\FUNC\\picture\\p"
    output_folder = "D:\\python\\FUNC\\picture\\p\\new"
    filters = []
    while True:
        print("Выберите фильтры для обработки (sharpen, sepia, resize), для завершения введите 'done'")
        user_input = input("Введите фильтр: ")
        if user_input == 'done':
            break
        if user_input in ["sharpen", "sepia", "resize"]:
            filters.append(user_input)
        else:
            print("Недопустимый фильтр. Пожалуйста, выберите из 'sharpen', 'sepia' или 'resize'.")

    if filters:
        process_images(input_folder, output_folder, filters)
    else:
        print("Не выбраны фильтры для обработки.")
